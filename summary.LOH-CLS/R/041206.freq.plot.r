...
# output to out1,  
# columns in out1: "t       white      black        B0.5 B0.25       total"
...
### plot out1 
 ...
 plot( out1[,2] ~ out1$t, ylab="s", xlab="t",type='l');

 #cols=rainbow( length(out1[1,]) );
 cols = c("green","black","red","purple","blue");
 y.max = max( out1[, 2:length(out1[1,])], na.rm=T ) * 1.5;
 y.min = min( out1[, 2:length(out1[1,])], na.rm=T ) + 1e-4;
 # par( yaxp=c(1e-4,20,1));
 plot(out1[,2] ~ out1$t,type='l',  col=cols[1], main=file, 
 ...
 labels = names( out1 );
 legend( 0, y.max * 0.8 , labels[2:length(labels)], col=cols, lty=1);
 dev.off();
