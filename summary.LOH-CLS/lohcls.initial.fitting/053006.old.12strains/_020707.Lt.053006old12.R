#020807 add Lmax
#020707 raw L calculation and TLmax estimation
...
#wrong order  parameters = c("strain", "Tc","wc", "Tmmax","Tg","wg","Tbmax","Td","wd","Tdmax","b.min","b.max","b0", "half.0.raw", "TLmax"); ##013107 change

#020707 switch b.max and b.min
...
######## calclulate T_g and w_g using Pb
 ...
 #020707  error out2[ss, c("b.min", "b.max", "b0", "half.0.raw")] = c(b.tmp[1], b.tmp[length(b.tmp)], out$Pb[1], out$R0.5.raw[1]  );
 out2[ss, c("b.max", "b.min", "b0", "half.0.raw")] = c(b.max, b.min, out$Pb[1], out$R0.5.raw[1]  );
 ...
 # out$L     = out$R0.5.raw / out$Pb;     ###This incorrect. 
 out$L[out$L==Inf] = NA; ###013107

 plot( out$R0.5 ~ out$t, main= paste(current_strain, "053006"), type='b' );
 plot( out$Rb   ~ out$t, main= paste(current_strain, "053006"), type='b' );
 plot( out$L    ~ out$t, main= paste(current_strain, "053006"), type='b' );
 ...
 out2$Lmax[ss] = tmp.L ; #020807 
 
 write.table(out2, "estimations.meng053006.020807.csv", row.names=F, quote=F, sep="\t");
