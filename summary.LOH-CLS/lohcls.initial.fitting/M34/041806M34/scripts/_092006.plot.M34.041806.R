#todo error bars are missing in the last two data points. 
rm(list=ls());
...
# ws = 1 / out$e.b ^ 2
 ...
 #fm.b = gnls( b ~ 0.30 - (0.30 - 0.008) / (1 + (t/T.g)^w ), start=list( T.g=5, w=2));
 fm.b = gnls( b ~ 0.30 - (0.30 - 0.008) / (1 + (t/T.g)^15 ), start=list( T.g=5) );
 fm.b
 ...
 fit.b = logistical.black( b.max, b.min, fm.b$coefficients[1], 15, t);
 # fit.b2 = logistical.black( b.max, b.min, fm.b$coefficients[1], 10, t);
 
 plot( out$Pb ~ out$t );
 lines( fit.b ~ t, col="blue");

###################overlay s, Pb, 
 postscript("092006.041806M34.s.Pb.ps", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
 
 plot( out$s ~ out$t, col="blue", xlab="t(days)",ylab="Viability", ylim=c(0, 1.2), pch=16 );
...
# par(new=T)
 ...
 lines( out$Pb ~ out$t, pch=15, axes=F,xlab="",ylab="", ylim=ylim );
...
###################overlay s, m, Pb, Rb
 postscript("091706.041806M34.m.Rb.ps", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
...
##################################
 postscript("091706.041806M34.Rb.R05.L.ps", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
