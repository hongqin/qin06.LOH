...
###################overlay s, Pb, 
 pdf("030707.041806M34.s.Pb.pdf", width=8, height=8)
...
###################overlay s, m, Pb, Rb
 pdf("030707.041806M34.m.Rb.pdf", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
...
##################################
 pdf("030707.041806M34.R05.pdf", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);

 out$g.e = genome.integrity(b.max, b.min,  T.g, w.g, out$t );
 out$R0.5   = out$R0.5.raw / out$g.e;                   #### 090706 change
 ylim = c(0, 0.1)
 plot( out$R0.5 ~ out$t, col="green", xlab="t (days)",ylab="Normalized half-black occurence", type='l',lty=2, ylim=ylim,xlim=xlim  );
 title (file);                                        ############change here
 ...
 #add labels here
 cols  =c("blue","green");
 labels=c("viability", "R0.5(t)")
 ltypes=c(2,2,1,1) 
 pch   =c( NA,NA,NA, 16)
 legend( 11 , 1  ,labels, col=cols, lty=ltypes, pch=pch);
 
 dev.off();


 ##################################
 pdf("030707.041806M34.L.pdf", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ltypes=c(2,1,1,1) 
 pch   =c( NA,NA,NA, 16)
 legend( 11 , 0.9  ,labels, col=cols, lty=ltypes, pch=pch);
 
 dev.off();


 ##################################
 pdf("030707.041806M34.Fb.pdf", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
