# log plots
#031908 switch b(t) and s(b) y axes
#050707 pdf size change
#050307 exp.id added
#050207 b0.5(t) renamed
#040807 add Tc, Tg diagram
#030707 add error bars to the last two data points. 
# the last 2 data points has only one plate, estimate using 10%

rm(list=ls());

library(nlme)
file = "M34.041806.tab";
exp.id = "M34.041806";  #050307 change
...
######## calclulate T_g and w_g using Pb
 ...
 fm.b = gnls( b ~ my.fun(t,T.g), start=list( T.g=5) );       fm.b
...
###################overlay s, Pb, 
 pdf("031908.041806M34.s.Pblog.pdf", width=6, height=6)
 ...
 #title (file);
 par(new=T);
 plot( out$s ~ out$t, col="blue", xlab="t(days)",ylab="", ylim=ylim, pch=16, xlim=xlim,axes=F );
 #title (file);                                        ############change here
 axis(4, at=seq(0, 1.2, by=0.2) )
 mtext( "Viability", 4, 2);
 ...
 ltypes=c(2,2) 
 pch   =c(16,15)
 legend( 9.5 , 0.6 ,labels, col=cols, lty=ltypes, pch=pch);
 text( 12, 0.65, exp.id);
 ...
 axis(2, at=c( 1E-3, 1E-2, 1E-1, 0.5 ) )
 mtext( "b(t)", 2, 2);
...
###################overlay s, m, Pb, Rb
 pdf("073107.041806M34.m.Rb.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 legend( 11 , max(fit.m)/2.2, labels, col=cols, lty=ltypes, pch=pch);
 text( 13,  max(fit.m)/2.2 + 0.1, exp.id);
...
##################################
 pdf("073107.041806M34.b0.5.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ltypes=c(2,2,1,1) 
 pch   =c( NA,NA,NA, 16)
 legend( 11 , 0.8  ,labels, col=cols, lty=ltypes, pch=pch);
 text( 13, 0.85, exp.id); 
 dev.off();


 ##################################
 pdf("073107.041806M34.L.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 plot( out$L ~ out$t, col="red", xlab="t(days)", ylab="L(t)",  pch=16, type='l', xlim=xlim,ylim=c(1E-3,1) ); 
 ...
 ##################################
 pdf("073107.041806M34.Fb.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ##################################
 pdf("073107.041806M34.Fb.F0.5.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ylim = c(min(tb.m$black)/2, max(tb.m$black)*1.5 );
 plot( tb.m$black ~ tb.m$t, col="black", type='l',lty=1, xlab="t (days)",ylab="Concentration of blacks", xlim=xlim, ylim=ylim, log='y');
 arrows( tb.m$t, (tb.m$black - e.fb), tb.m$t, (tb.m$black + e.fb), length=0.1, angle=90,code=3, lty=1, col="red" );
 points( tb.m$t, tb.m$black, pch=16);
 ...
 t = seq(0, max(out$t),by=0.1);
 fit.s = logistical.viability ( fm.s$coefficients[1], fm.s$coefficients[2], t );
 plot( fit.s ~ t, col="blue", type='l',lty=2, axes=F, xlab="",ylab="", xlim=xlim, log='y'  );
 ...
 legend( 9 , 0.55  ,labels, col=cols, lty=ltypes, pch=pch);
 text( 12.5, 0.75, exp.id); 
