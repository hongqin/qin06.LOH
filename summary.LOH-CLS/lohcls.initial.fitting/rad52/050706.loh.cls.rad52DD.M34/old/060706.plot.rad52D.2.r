...
rm(list=ls());
#postscript("060206.loh.plot.rad52.1.ps",width=8, height=8);
...
#generate tb.m
 ...
 previous.row = 1;
 for( i in 1:n) {   # for( i in 1: length(tb.m[,1]) ) {
   tb.m[i,1]  = tb2$t[ row.num[i] ]
   for( j in 2: 10 ) {
     # tb.m[i, j] = mean( tb2[ c(3*i-2, 3*i-1, 3*i), j+3] )
     tb.m[i, j] = mean( tb2[ previous.row : row.num[i], j+3] )
   }
   tb.m$total[i] = sum( tb.m[i, 2:10], na.rm=T );
   previous.row = row.num[i] + 1;
 }
