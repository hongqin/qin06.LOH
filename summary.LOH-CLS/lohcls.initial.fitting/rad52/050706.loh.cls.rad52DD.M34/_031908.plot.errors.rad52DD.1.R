#031908 switch b(t) s(t) 
#080708 log
#050707 pdf size to 6x6
#022007Tue v1

rm(list=ls());

library(nlme)
file = "050706.rad52D.1.tab";
...
######## calclulate T_g and w_g using Pb
 ...
 b = out$Pb
 t = out$t;
 b.min = min( b ); b.min  # 0.1311410
 b.max = max( b ); b.max  # 0.3531409

 #####   Pr[B>=b] = b.max - (b.max - b.min) /( 1 + ( t / T )^ w ) ########formula for cumulative Pr(b)
 fm.b = gnls( b ~ 0.3531409 - (0.3531409 - 0.1311410) / (1 + (t/T.g)^w ), start=list( T.g=2, w=2) );
 fm.b
 ...
 ###################overlay s, m, Pb, and R0.5, half/full, mortality
 ###################022007, remove R0.5 and half/full
 pdf("031908.050706rad52D.1.log.pdf", width=6, height=6)
 ...
 axis(4, at=seq(0, 1.0, by=0.2) )
 mtext( "Viability", 4, 2);
 axis(1, at=seq(0,17, by=4));
 box();

 #plot( out$s ~ out$t, col="blue", xlab="t(days)",ylab="Viability", ylim=c(0.0001, 1.1),xlim=xlim, pch=16,log='' );
 ...
 ltypes=c(2,2) 
 pch   =c(16,15)
 legend( 8 , 1 ,labels, col=cols, lty=ltypes, pch=pch);
 text( 10, 1.05, "rad52D/D 050706#1");

 par(new=T); 
 plot( out$Pb ~ out$t, pch=15, axes=F, log='y',xlab='',ylab='',xlim=xlim,ylim=c(1E-1,0.6) );
 arrows( out$t, (out$Pb - out$e.b), out$t, (out$Pb + out$e.b), length=0.1, angle=90,code=3, lty=1 );
 lines( fit.b ~ t, lty=2);
 axis(2, at=c( 1E-3, 1E-2, 1E-1, 0.2, 0.4, 0.6, 1) );
 mtext( "b(t)", 2, 2);

 dev.off();

##################################
 pdf("072507.050706rad52D.1.overlay.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
...
# lines( fit.L ~ t, col="red", lty=2);
 ...
 # labels=c("viability","Pr(b)","half/full")  ### 022007 change
 labels=c("viability","b(t)")
 ltypes=c(2,2,2) 
 pch   =c( 16, 15,16)
