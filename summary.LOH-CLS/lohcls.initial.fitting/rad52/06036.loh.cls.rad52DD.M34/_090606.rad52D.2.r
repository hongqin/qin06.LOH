...
######## calclulate T_g and w_g using Pb
 ...
 b = out$Pb
 t = out$t;
 b.min = min( b ); b.min  # 0.11118851 #############change this
 b.max = max( b ); b.max  # 0.2879445  ##########change here

 #####   Pr[B>=b] = b.max - (b.max - b.min) /( 1 + ( t / T )^ w ) ########formula for cumulative Pr(b)
 fm.b = gnls( b ~ 0.2879445 - (0.2879445 - 0.11118851) / (1 + (t/T.g)^w ), start=list( T.g=7.5, w=2) );
 fm.b
 plot( fm.b );
 ...
 postscript("090806.060306rad52D.2.ps", width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
 
 plot( out$s ~ out$t, col="blue", xlab="t(days)",ylab="percentage" );
 title ("rad52DD 060306 #2");
...
######calculate Rb = d(Pb)/dt
 ...
 ltypes=c(2,2,2,1) 
 legend( 5 , max(out$L[2:length(out$L)]) * 0.9  ,labels, col=cols, lty=ltypes);
