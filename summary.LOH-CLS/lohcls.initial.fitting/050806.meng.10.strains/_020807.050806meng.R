# add Lmax
#020707Wed This fitting method does quite work for the batch experiemnt. I have to visually estimate TLmax.
#020607Tue TLmax by fitting
...
parameters = c("strain", "Tc","wc", "Tmmax","Tg","wg","Tbmax","Td","wd","Tdmax","b.max","b.min","b0", "half.0.raw", "TLmax", "Lmax"); ##013107 change
out2 = data.frame( matrix( nrow= length(strains), ncol=length(parameters) ) );
names(out2) = parameters;

pdf( paste("020807", file, "pdf", sep="."), width=8, height=8 ); 
...
# if(  current_strain == "M5" ) {  out$Pb[c(2)] = NA;  }
 ...
 b.tmp = rev(b.tmp); 
 b.max = b.tmp[ 2 ] /2 + b.tmp[ 2] /2  #use the 2nd one
 # b.min = b.tmp[1]/2 + b.tmp[2]/2;
 ...
 ###020607 change
 fit.Pb1  = logistical.black(b.max, b.min, out2$Tg[ss], out2$wg[ss], tt );  #020607 change
 fit.Pb2  = (1-fit.g )/2;   						#020607 change
 plot( fit.Pb1 ~ fit.Pb2, main= paste(current_strain,"050806") );  #020607 change
 ...
 out$L     = out$R0.5 / out$Pb;     ###change here Jan 31 2007
 out$L[out$L==Inf] = NA; ###013107

 plot( out$R0.5 ~ out$t, main= paste(current_strain, "050806"), type='b' );
 plot( out$Rb   ~ out$t, main= paste(current_strain, "050806"), type='b' );
 plot( out$L    ~ out$t, main= paste(current_strain, "050806", "raw.L"), type='b',  );
 ...
 #020607 change
 fit.half = fd( tt, out2$Td[ss], out2$wd[ss] );
 fit.L = fit.half / fit.Pb1; 
 plot( fit.L ~ tt,    main= paste(current_strain,"050806"), type='l', col="blue" ); 
 plot( fit.half ~ tt, main= paste(current_strain,"050806"), type='l', col="red");
 plot( fit.Pb1 ~ tt,  main= paste(current_strain,"050806"), type='l', col="black");
 ...
 ####
 #### 020607 find peak of fit.L
 #tb.slope <- data.frame( cbind(tt, fit.L));

 #slope <- function( y, x ) {
 # m1 <- lm ( y ~ x );
 # ret <- as.numeric( m1$coefficient[2] );
 #}

 #for ( i in 3:(length(tt)-2) ) {
 #  t0 <- i - 2;
 #  t1 <- i + 2;
 #  y <- fit.L[ t0:t1];
 #  x <- tt[ t0:t1];
 #  b <- slope( y, x );
 #  tb.slope$t.avg[t0] <- mean(x);
 #  tb.slope$slope[t0] <- b;
 #}

 #plot( tb.slope$slope ~ tb.slope$t.avg)
 #par(new=T);
 #plot( fit.L ~ tt, type='l', lwd=2, col="blue", axes=F);

 #find TLmax
 #t.max.deriv = tb.slope$t.avg[ tb.slope$slope == max( tb.slope$slope ) ];
 #sub.L = tb.slope$fit.L[ tb.slope$tt > t.max.deriv ]
 #TLmax = tb.slope$tt[ tb.slope$fit.L == max(sub.L) ]
 ...
 #### old 013107, simple way to find TLmax
 tmp.L = max(out$L, na.rm=T); 
 TLmax = out$t[ out$L == tmp.L ]
 TLmax = TLmax[! is.na(TLmax)]
 out2$TLmax[ss] = min(TLmax);
 out2$Lmax[ss] = tmp.L

 write.table(out2, "estimations.meng050806.020807.csv", row.names=F, quote=F, sep="\t");
