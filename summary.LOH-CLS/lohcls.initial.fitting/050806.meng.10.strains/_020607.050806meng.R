...
parameters = c("strain", "Tc","wc", "Tmmax","Tg","wg","Tbmax","Td","wd","Tdmax","b.min","b.max","b0", "half.0.raw", "TLmax"); ##013107 change
out2 = data.frame( matrix( nrow= length(strains), ncol=length(parameters) ) );
names(out2) = parameters;

pdf( paste("020607", file, "pdf", sep="."), width=8, height=8 ); 
...
# if(  current_strain == "M5" ) {  out$Pb[c(2)] = NA;  }
 ...
 write.table(out2, "estimations.meng050806.020607.csv", row.names=F, quote=F, sep="\t");
