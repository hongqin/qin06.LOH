...
############################ loop by strains
 ...
 ss = 1;
  ...
  postscript( paste(file, current_strain, "ps", sep="."), width=8, height=8); 

  ## here takes tb2 by strains
  tb.sub = tb[as.character(tb$Strain)==current_strain, 2:length(tb[1,])]; 
  labels = names( tb.sub );  ##091406

  ## manuall change here !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  tb.sub$black[c(13,14)] = 1;
 ...
 for( j in tmp.indice["white"] : length(labels) ) {
   for( i in  1: length(tb2[,1]) ) {
     ...
     tb2[i,j] = tb2[i,j] * tb2[i,"D1"] * tb2[i,"D2"];
   }
 }
...
######## calclulate T0.5 w_s
 ...
 plot( out$s ~ out$t, log='y', main=paste(file, current_strain) );
 lines( fit.s ~ t, col="blue");
 par(new=T);
 plot( fit.m ~ t, col="brown", axes=F, type='l', xlab="",ylab="");
...
######## calclulate T_g and w_g using Pb
 ...
 b = out$Pb
 t = out$t;
 b.min = min( b ); b.min  # 0.001463058
 b.max = max( b ); b.max  # 0.5714286

 #####   Pr[B>=b] = b.max - (b.max - b.min) /( 1 + ( t / T )^ w ) ########formula for cumulative Pr(b)
 fm.b = gnls( b ~ 0.5714286 - (0.5714286 - 0.001463058) / (1 + (t/T.g)^w ), start=list( T.g=4, w=5) );
 fm.b
...
##################################3
 #postscript( paste(file, current=strain, "overlay", "ps" sep="."),  width=8, height=8)
 par(mar=c(5,4,4,4)+0.1);
 
 plot( out$s ~ out$t, col="blue", xlab="t(days)",ylab="percentage", ylim=c(0, 1.1), pch=16 );
 title (paste(file, current=strain, date(), sep=" "));  
 arrows( out$t, (out$s - out$e.s), out$t, (out$s + out$e.s), length=0.1, angle=90,code=3, lty=1, col="blue" );
