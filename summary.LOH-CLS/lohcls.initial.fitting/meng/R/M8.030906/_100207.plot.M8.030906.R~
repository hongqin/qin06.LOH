...
###################overlay s, Pb, 
 pdf( paste( "072207",exp.id,"s.Pb.pdf", sep="."), width=6, height=6)
...
# par(new=T)
 ...
 ltypes=c(2,2) 
 pch   =c(16,15)
 legend( 7 , 0.8 ,labels, col=cols, lty=ltypes, pch=pch);
 text( 8, 0.85, exp.id);
...
# pdf("050707.030906YPS163.m.Rb.pdf", width=6, height=6)
 pdf( paste( "072207",exp.id,"m.Rb.pdf", sep="."), width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 legend( 0.5 , max(fit.m)/1.7, labels, col=cols, lty=ltypes, pch=pch);
 text( 2.5,  max(fit.m)/1.7 + 0.05, exp.id);
...
##################################
 ...
 ylim = c(0, max(out$L[2:length(out$L)], na.rm=T)*1.7 )

 plot( out$L ~ out$t, col="red", xlab="t(days)", ylab="L(t)", pch=16, type='l', xlim=xlim,ylim=ylim ); 
 ...
 ltypes=c(2,1,1,1) 
 pch   =c( NA,16,NA, 16)
 legend( 6.5 , 0.8  ,labels, col=cols, lty=ltypes, pch=pch);
 text( 8, 0.85, exp.id); 
