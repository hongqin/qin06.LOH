#072307 M32 needs some modification for the plots
# its b(t) is so low that is has to be plotted on a different scale.

# modified from _050307.plot.M34.041806.R
#050307 exp.id added
...
#######calculate s , g
 ...
 plot( out$s ~ out$t , type='l', main= file, col="blue", log='y', ylim=c(1E-5,1.5));
...
######## calclulate T_g and w_g using Pb
 ...
 b.min = 0.011;        ##############change this 
 b.max = 0.022;        ###############change this
 T.g = 7.8;
 #w.g = 5;		###############change this

 my.fun = function (t, w.g) { b.max - (b.max - b.min) /( 1 + ( t / T.g )^ w.g ); }
...
###################overlay s, Pb, 
 pdf( paste( "072307",exp.id,"s.Pb.pdf", sep="."), width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);

 xlim = c(0,16)

 plot( out$s ~ out$t, col="blue", xlab="t (days)",ylab="Viability", ylim=c(0, 1.2), pch=16, xlim=xlim );
 ...
 ltypes=c(2,2) 
 pch   =c(16,15)
 legend( 10 , 0.4 ,labels, col=cols, lty=ltypes, pch=pch);
 text( 12, 0.45, exp.id);
 ...
 plot( out$t, out$Pb, pch=15, xlab="",ylab="", ylim=ylim, type='p',axes=F, xlim=xlim ); 
 ...
 mtext( "Tc",side=1,at=c(T.c - 0.5) );
 mtext( "Tg",side=1,at=c(T.g + 0.5) );
...
###################overlay s, m, Pb, Rb
# pdf("050707.030906YPS163.m.Rb.pdf", width=6, height=6)
 pdf( paste( "072307",exp.id,"m.Rb.pdf", sep="."), width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 legend( 9.5 , max(fit.m)/2.7, labels, col=cols, lty=ltypes, pch=pch);
 text( 13.5,  max(fit.m)/2.7 + 0.05, exp.id);
...
##################################
 #pdf("072207.030906YPS163.b0.5.pdf", width=6, height=6)
 pdf( paste( "072207",exp.id,"b0.5.pdf", sep="."), width=6, height=6)
 ...
 ylim = c(0, 0.04)
 plot( out$R0.5 ~ out$t, col="green", xlab="t (days)",ylab="Normalized half-black percentage", type='l',lty=2, ylim=ylim,xlim=xlim  );
 # title (file);                                        ############change here
 ...
 text( 12.5, 0.85, exp.id); 
 dev.off();


 ##################################
 #pdf("072207.030906YPS163.L.pdf", width=6, height=6)
 pdf( paste( "072207",exp.id,"L.pdf", sep="."), width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ltypes=c(2,1,1,1) 
 pch   =c( NA,NA,NA, 16)
 legend( 10 , 0.9  ,labels, col=cols, lty=ltypes, pch=pch);
 text( 12, 0.95, exp.id );
 dev.off();


 ##################################
 #pdf("072207.030906YPS163.Fb.pdf", width=6, height=6)
 pdf( paste( "072207",exp.id,"Fb.pdf", sep="."), width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ylim = c(min(tb.m$black)/2, max(tb.m$black)*1.5 );
