#022807 add freq of black in the plot

#20807 find Lmax, and TLmax
...
parameters = c("expt", "Tc","wc", "Tmmax","Tg","wg","Tbmax","Td","wd","Tdmax","b.max","b.min","b0", "half.0.raw", "TLmax", "Lmax");
out2 = data.frame( matrix( nrow= length(expts), ncol=length(parameters) ) );
names(out2) = parameters;

pdf( paste("022807", filelist, "pdf", sep="."), width=8, height=8 ); 
...
for( ss in 1:length(files) ) {
 ...
 out$fb     = tb.m$black / tb.m$black[1] ###0220807 check the relative freq of full-blacks

 out$R0.5.raw   = tb.m$B0.5 /tb.m$total; #needs to be adjusted by g.e
  # out$e.b05  =  out$R0.5.raw * ( tb.m$sd.b05 / tb.m$B0.5)

 ### plot of the raw data 
 ymax = max( 1, max(out$fb) ); #022807 
 if(is.na(ymax)) { ymax=2; } 

 plot( out$s ~ out$t , type='l', main= paste(current_expt), col="blue",  ylim=c(1E-5,ymax) );
 lines( out$Pb ~ out$t, col="black");
 lines( out$fb ~ out$t, col="red"); ### 022807
 labels = c("viability","Pb", "fb");
 ltypes = c(1,1, 1);
 legend( (max(out$t)*0.7 ), 0.8, labels, col=c("blue", "black", "red"), lty = ltypes);

 t.test( out$s, out$fb, pairwise=T);  ### 022807
...
######## calclulate T_g and w_g using Pb
 ...
 if(  current_expt == "101S.030906.tab" ) { #out$Pb[out$Pb==max(out$Pb, na.rm=T)] = NA
   out$Pb[c(9:11)] = NA;   }
 ...
 plot( out$L    ~ out$t, main= paste(current_expt), type='b' );
 ...
 if( td.max.pos < length(out$t) ) {
    max.range = c( max.range, td.max.pos +1); #source of over-estimation?
 } else {
    max.range = c( td.max.pos -2, max.range);
 }
 y = out$R0.5[max.range];
 y[is.na(y)] = 0;
 out2$Tdmax[ss] = sum( out$t[max.range] * y ) / sum(y)  #weighted average 
     ##013107 note, the above weighted average approach works for sharp peaks
     ##   The weighted average will over-estimate the peak if it is actually a plateau. 

 #### 013107 added find the peak of L(t)
 ...
 write.table(out2, "estimations.meng.022807.csv", row.names=F, quote=F, sep="\t");
