...
###################overlay s, Pb, 
 pdf("050707.032906M5.s.Pb.pdf", width=6, height=6)
...
###################overlay s, m, Pb, Rb
 pdf("072207.032906M5.m.Rb.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
...
##################################
 pdf("072207.032906M5.b0.5.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ltypes=c(2,2,1,1) 
 pch   =c( NA,NA,NA, 16)
 legend( 12 , 0.9  ,labels, col=cols, lty=ltypes, pch=pch);
 text( 14, 0.95, exp.id); 
 dev.off();


 ##################################
 pdf("072207.032906M5.L.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 plot( out$L ~ out$t, col="red", xlab="t(days)", ylab="L(t)",  pch=16, type='l', xlim=xlim,ylim=c(0,5) ); 
...
##################################
 pdf("072507.032906M5.Fb.F0.5.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
