#060908 by4743 for plosOne supporting figure
...
#generate tb.m
 ...
 tb.m.original = tb.m;  ##########060908 backup
...
######## calclulate T0.5 w_s
 t= out$t
 s= out$s
 fm.s <- gnls( s ~ 1 /( 1 + ( t / v )^ w ) , start = list( v = 14, w = 2.5 )  );
...
######## calclulate T_g and w_g using Pb
 ...
 b = out$Pb
 t = out$t;
 b.min = min( b ); b.min  # 0.004691283
 b.max = max( b ); b.max  # 0.0403286
 ...
 fm.b = gnls( b ~ my.fun(t, T.g, w.g), start=list( T.g=20, w.g=2) );              
 fm.b
...
###################overlay s, Pb, 
 pdf("060908.032906BY4743.s.Pblog.pdf", width=6, height=6)
...
###################overlay s, m, Pb, Rb
 pdf("060908.032906BY4743.r.m.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
...
##################################
 pdf("060908.032906BY4743.b0.5log.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ltypes=c(1,2,1,1) 
 pch   =c( NA,16,NA, 16)
 legend( 12 , 0.3  ,labels,  lty=ltypes, pch=pch);
 text( 14, 0.35, exp.id); 
 dev.off();


 ##################################
 pdf("060908.032906BY4743.L.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
 ...
 ##################################
 pdf("060908.032906BY4743.Fb.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);
...
##################################
 pdf("060908.032906BY4743.Fb.F0.5.pdf", width=6, height=6)
 par(mar=c(5,4,4,4)+0.1);

 ####### 060908 remove outliers in tb.m
 tb.m = tb.m.original;
 tb.m$t[c(11:13,15)] = NA;
 tb.m$sd.b[c(16,17)]    =  tb.m$sd.b[c(16,17)] * 0.3  
 tb.m$sd.b05[c(16,17)]  =  tb.m$sd.b05[c(16,17)] * 0.3  

 tb.m = tb.m[ (! is.na(tb.m$t) ), ]
 tb.m[ c(10:20),c("black","t","sd.b")] #check 

 e.fb = tb.m$sd.b; 
 #e.fb[2] = e.fb[2]*0.6;
 #e.fb[c(16,17)] =  e.fb[c(16,17)] * 0.1; ####???
 y.scale = 1E3;
 ...
 #e.05[c(14,16,17)] =  e.05[c(14,16,17)] * 0.3; ####???
 ...
 legend( 20 , 0.9  ,labels, col=cols, lty=ltypes, pch=pch);
 text( 27, 0.95, exp.id); 
