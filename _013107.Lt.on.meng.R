#Jan 31, 07 Recalculate L(t) using P0.5/Pb, estimate T.Lmax, T.Lmax.fitted
...
parameters = c("expt", "Tc","wc", "Tmmax","Tg","wg","Tbmax","Td","wd","Tdmax","b.min","b.max","b0", "half.0.raw", "TLmax"); ##013107 change
out2 = data.frame( matrix( nrow= length(expts), ncol=length(parameters) ) );
names(out2) = parameters;

pdf( paste("013107", filelist, "pdf", sep="."), width=8, height=8 ); 
...
######## calclulate T_g and w_g using Pb
 ...
 write.table(out2, "estimations.meng.013107.csv", row.names=F, quote=F, sep="\t");
